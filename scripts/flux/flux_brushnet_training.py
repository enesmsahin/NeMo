...
from nemo.collections.diffusion.models.flux_brushnet.model import FluxBrushNetConfig, MegatronFluxBrushNetModel
...
def flux_brushnet_training() -> run.Partial:
    """Flux Brushnet Training Config"""
    return run.Partial(
        llm.train,
        model=run.Config(
            MegatronFluxBrushNetModel,
            flux_params=run.Config(FluxModelParams),
            flux_controlnet_config=run.Config(FluxBrushNetConfig),
        ),
...
def datamodule_test(tp=2, devices=8) -> run.Partial:
    ...
    recipe.model.flux_params.vae_config = run.Config(
        AutoEncoderConfig, ckpt='/workspace/weights/ae.safetensors', ch_mult=[1, 2, 4, 4], attn_resolutions=[]
    )
    recipe.model.flux_params.vae_config = run.Config(
    ...
    recipe.data = brushnet_datamodule('/workspace/tarfiles_val/tarfiles_val')
